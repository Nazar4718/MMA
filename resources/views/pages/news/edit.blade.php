...
@section("content")
    <div class="isolate bg-white px-6 py-24 sm:py-32 lg:px-8">
        ...
        <form action="{{route('news.update', ['news'=>$news->id])}}" method="POST" class="mx-auto mt-16 max-w-xl sm:mt-20">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 gap-x-8 gap-y-6 sm:grid-cols-2">
                <div class="sm:col-span-2">
                    <label for="title" class="block text-sm/6 font-semibold text-gray-900">Edit Title</label>
                    <div class="mt-2.5">
                        <input type="text" value="{{old('title', $news)}}" name="title" id="title"
                ...
                <div class="sm:col-span-2">
                    <label for="description" class="block text-sm/6 font-semibold text-gray-900">Edit Description</label>
                    <div class="mt-2.5">
                        <input type="text" value="{{old('description', $news)}}" name="description" id="description"
                               class="block w-full rounded-md bg-white px-3.5 py-2 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600">
                    </div>
                    @error('description')
                    <div class="alert alert-danger">{{ $message }}</div>
                ...
                <div class="grid grid-cols-1 gap-x-8 gap-y-6 sm:grid-cols-2">
                    <div class="sm:col-span-2">
                        <label for="date_published" class="block text-sm/6 font-semibold text-gray-900">Edit Date</label>
                        <div class="mt-2.5">
                            <input type="text" value="{{old('date_published', $news)}}" name="date_published" id="date_published"
                                   class="block w-full rounded-md bg-white px-3.5 py-2 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600">
                        </div>
                    ...
                    <div class="grid grid-cols-1 gap-x-8 gap-y-6 sm:grid-cols-2">
                        <div class="sm:col-span-2">
                            <label for="author" class="block text-sm/6 font-semibold text-gray-900">Edit Author</label>
                            <div class="mt-2.5">
                                <input type="text" value="{{old('author', $news)}}" name="author" id="author"
                                       class="block w-full rounded-md bg-white px-3.5 py-2 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600">
                            </div>
                            @error('author')
                            <div class="alert alert-danger">{{ $message }}</div>
