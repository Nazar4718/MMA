...
@section("content")
    <div class="isolate bg-white px-6 py-24 sm:py-32 lg:px-8">
        ...
        <div class="mx-auto max-w-2xl text-center">
            <h2 class="text-4xl font-semibold tracking-tight text-balance text-gray-900 sm:text-5xl">Create Post</h2>
        </div>
        <form action="{{route('blog.store')}}" method="POST" class="mx-auto mt-16 max-w-xl sm:mt-20">
            @csrf
            <div class="grid grid-cols-1 gap-x-8 gap-y-6 sm:grid-cols-2">
                ...
                <div class="sm:col-span-2">
                    ...
                    <div class="mt-2.5">
                        ...
                        <input type="checkbox" value="1" name="is_public" id="is_public" {{old('is_public')=='1' ? 'checked' : ''}} class="block rounded-md bg-white px-3.5 py-2 text-base text-gray-900 outline-1 -outline-offset-1 outline-gray-300 placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600">
                    </div>
                    @error('is_public')
                    <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
